...
import { isCatalog, type Catalog, type CatalogColor, type CatalogProduct } from "@/lib/catalog";
import "../exact-storefront/exact.css";
...
const CART_KEY = "silent-script-exact-cart-v2";
...
const copy = {
  uz: {
    ...
    thread: "Ip rangi",
    notebook: "Daftar",
    withNotebook: "Daftar bilan",
    withoutNotebook: "Daftarsiz",
    pages: "bet",
    paper: "Qog‘oz turi",
  ...
  ru: {
    ...
    thread: "Цвет нити",
    notebook: "Блокнот",
    withNotebook: "С блокнотом",
    withoutNotebook: "Без блокнота",
    pages: "стр.",
    paper: "Тип бумаги",
...
function isCowhide(color: CatalogColor): boolean {
  const value = [color.id, color.name.uz, color.name.en, color.name.ru].join(" ").toLowerCase();
  return value.includes("cowhide") || value.includes("cow hide") || value.includes("коров") || value.includes("sigir");
}

function availableColors(product?: CatalogProduct): CatalogColor[] {
  const fallback: CatalogColor = {
    id: "brown",
    hex: "#6b412f",
    name: { uz: "Jigarrang", en: "Brown", ru: "Коричневый" },
  };
  if (!product?.colors?.length) return [fallback];
  const filtered = product.colors.filter((color) => !isCowhide(color));
...
function CartIcon({ size = 20 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M3 4h2l1.65 9.02a2 2 0 0 0 1.97 1.64h7.92a2 2 0 0 0 1.95-1.55L20 7H6.1" stroke="currentColor" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" />
...
export default function BusinessStorefront() {
  ...
  const leatherOptions = availableColors(selectedProduct);
  ...
  const supportsNotebook = Boolean(selectedProduct?.supportsInsert);
  const selectedPrice = (selectedProduct?.basePrice || 0) + (supportsNotebook && includeNotebook ? selectedProduct?.insertPrice ?? 0 : 0);
  ...
  useEffect(() => {
    ...
    const colors = availableColors(selectedProduct);
    if (!colors.some((color) => color.id === leatherId)) setLeatherId(colors[0]?.id || "");
    if (!selectedProduct.supportsInsert && includeNotebook) setIncludeNotebook(false);
  }, [selectedProduct, leatherId, includeNotebook]);
  ...
  function selectProduct(product: CatalogProduct) {
    ...
    setLeatherId(availableColors(product)[0]?.id || "");
  ...
  function addToCart() {
    ...
    const key = [selectedProduct.id, selectedLeather.id, selectedThread.id, includeNotebook ? "with-notebook" : "cover-only", includeNotebook ? paper : "no-paper"].join("-");
    setCart((current) => {
      ...
      return [...current, {
        ...
        includeNotebook: supportsNotebook && includeNotebook,
        insertPages: selectedProduct.insertPages,
        paper,
      }];
    });
    setToast(t.added);
  ...
  async function submitOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const lines = cart.map((item, index) => {
      const notebookText = item.includeNotebook
        ? `${t.withNotebook}${item.insertPages ? ` (${item.insertPages} ${t.pages})` : ""}`
        : t.withoutNotebook;
      return [
        ...
        `${t.notebook}: ${notebookText}`,
        ...(item.includeNotebook ? [`${t.paper}: ${copy[locale][item.paper]}`] : []),
    ...
    <main className="ex-site">
      ...
      <header className="ex-header">
        ...
        <div className="ex-actions">
          ...
          <button className="ex-cart-button" type="button" onClick={() => setCartOpen(true)} aria-label={t.cart}><CartIcon /><b>{cartCount}</b></button>
      ...
      <section className="ex-products" id="products">
        ...
        <div className="ex-product-grid">
          {products.map((product, index) => {
            const color = availableColors(product)[0]?.hex || "#6a4632";
        ...
        <section className="ex-builder" id="design">
          ...
          <div className="ex-builder-grid">
            <div className="ex-options">
              ...
              {supportsNotebook ? (
                <fieldset>
                  <legend>4. {t.notebook}</legend>
                  <div className="ex-paper-options" style={{ gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
                    <button type="button" className={!includeNotebook ? "is-active" : ""} onClick={() => setIncludeNotebook(false)}>{t.withoutNotebook}</button>
                    <button type="button" className={includeNotebook ? "is-active" : ""} onClick={() => setIncludeNotebook(true)}>{t.withNotebook} (+{formatUzs(selectedProduct.insertPrice ?? 0)})</button>
                  </div>
                </fieldset>
              ) : null}
              {supportsNotebook && includeNotebook ? (
                <fieldset>
                  <legend>5. {t.paper}</legend>
            ...
            <aside className="ex-summary">
              ...
              <dl>
                ...
                {supportsNotebook ? <div><dt>{t.notebook}:</dt><dd>{includeNotebook ? `${t.withNotebook}${selectedProduct.insertPages ? ` (${selectedProduct.insertPages} ${t.pages})` : ""}` : t.withoutNotebook}</dd></div> : null}
                {supportsNotebook && includeNotebook ? <div><dt>{t.paper}:</dt><dd>{t[paper]}</dd></div> : null}
              </dl>
              <p><b>{t.price}: {formatUzs(selectedPrice)}</b></p>
              <button type="button" onClick={addToCart}><CartIcon size={17}/>&nbsp; {t.add}</button>
              ...
              {!cart.length ? <p className="ex-empty">{t.empty}</p> : cart.map((item) => (
                <article key={item.key}>
                  ...
                  <div>
                    ...
                    <p>{item.size} · {item.leatherName} · {item.threadName}</p>
                    <p>{item.includeNotebook ? `${t.withNotebook}${item.insertPages ? ` · ${item.insertPages} ${t.pages}` : ""}` : t.withoutNotebook}</p>
                    <strong>{formatUzs(item.price)}</strong>
                    <div className="ex-qty"><button type="button" onClick={() => changeQuantity(item.key, -1)}>−</button><span>{item.quantity}</span><button type="button" onClick={() => changeQuantity(item.key, 1)}>+</button><button type="button" onClick={() => setCart((current) => current.filter((line) => line.key !== item.key))}>{t.remove}</button></div>
                  </div>
                </article>
              ))}
            </div>
